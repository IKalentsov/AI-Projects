...
**Ограничения среды агента при прогоне тестов** (подробности — скилл `dotnet-verify-in-sandbox`):
...
**Ограничения среды агента** (проверено 2026-09-21; подробности — скилл
`dotnet-verify-in-sandbox`): без `-m:1` и `restore`, и `build` падают молча с кодом 1;
...
|------|-------------|
| `ARCHITECTURE.md` | этот файл |
| `memory-bank/snapshot.md` | инвентарь проекта и реестр находок — быстрее, чем обход файлов |
...
- [ ] **Перевыпустить токен бота у @BotFather** — текущий был передан открытым текстом и считается скомпрометированным
- [ ] `01-01`: убрать из проекта всё, связанное с платным источником, и перейти на Open-Meteo;
      вынести город в настройки
- [ ] `01-02`: завести тестовые проекты (unit + architecture) и первые тесты
- [ ] Автозапуск периодической отправки после перезапуска приложения (находка `F-02`)
...
- [ ] Привести в порядок обоснования в `backend/.globalconfig` (находка `F-08`)
