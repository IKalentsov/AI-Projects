...
| F-16 | `Web` ссылается на `Domain`, не используя ни одного его типа, — та же история, что `F-10`. Ссылка выброшена компилятором, поэтому архитектурный тест её не видит и правило «`Web` не зависит напрямую от `Domain`» выполняется формально | `WeatherBot.Web.csproj`, §«Направления зависимостей» | низкая |
| F-17 | Два режима округления в одном `WeatherInfo`: температура и «ощущается» — `(int)Math.Round(x)` (банковское), влажность и давление — `MidpointRounding.AwayFromZero`. На значении `16.5` это даёт `16` для температуры и `17` для влажности | `Infrastructure/Weather/OpenMeteoWeatherProvider.cs:101-104` | низкая |

Закрытые (для справки, действий не требуют): F-01 — платный тариф делал сводку неотправляемой;
F-03 — тесты не были написаны; закрыта тикетами `01`–`03` (122 unit-теста + 11 архитектурных);
F-04 — проект не был в git; F-05 — captive dependency `IWeatherProvider`; F-07 — «Москва»/«МСК»
константами; F-12 — `ObservedAt` вместо времени наблюдения из API; F-15 — ответ Open-Meteo
не разбирался: snake_case-имена API не мапились на PascalCase-свойства контракта, поэтому
провайдер не падал, а молча возвращал нули (0 °C, 0 % влажности, ветер 0, смещение 0); найден и
исправлен тикетом `01`; F-14 — три граничных дефекта маппера и форматтера (минуты смещения
терялись, отрицательные осадки давали «очень сильные», влажность усекалась); закрыта тикетом `03`;
F-18 — координаты уходили в Open-Meteo в культуре процесса (`latitude=55,7558`), API отвечал `400`
и погода не приходила вовсе; найдена живым запуском, закрыта тикетом `07` — координаты
форматируются инвариантно, регрессия закреплена тестом при культуре `ru-RU`.
Закрыты задачами `01-01`, `01-03`, `01`, `03`, `07`.
...
- [x] `.scratch/weather-provider`: тикеты `01`–`06` сданы — тестовый контур закрыт, граничные
      дефекты исправлены, архитектурные тесты защищены канарейкой (`122` unit-теста +
      `13` архитектурных; закрыты находки `F-14`, `F-15`)
- [x] `F-18`: координаты в URL Open-Meteo форматируются инвариантно (тикет `07`) — регрессия
      закреплена тестом при культуре `ru-RU`; полная сборка Release — `0/0`, `123` + `13` тестов
- [ ] Повторить живой прогон: `/api/bot/start`, `send-now`, приход сводки в канал по расписанию
- [ ] Решить судьбу пакетов `TngTech.ArchUnitNET` и `TngTech.ArchUnitNET.xUnitV3`: объявлены,
      но не используются — тесты слоёв идут через `Assembly.GetReferencedAssemblies()`
...
- [ ] Разобрать остальные находки реестра (`F-06`, `F-09`, `F-10`, `F-11`, `F-16`, `F-17`)
