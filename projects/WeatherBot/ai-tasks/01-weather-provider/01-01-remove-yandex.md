...
**Open-Meteo**: авторизации у API нет, ключ не нужен. Город выносится в настройки
(`OpenMeteo:City`, по умолчанию «Москва»).
...
- [ ] **Провайдер погоды резолвится**: поиск `AddHttpClient` по `src/**` находит совпадение.
      Это временная статическая проверка вместо теста — сборка **не ловит** незарегистрированную
      зависимость (в первой версии задачи `IHttpClientFactory` остался незарегистрированным при
      зелёной сборке). Настоящий тест на композицию появится в `01-02`.
...
**и** поиск по `yandex|яндекс` в дереве решения (`backend/WeatherBot`, без `bin`/`obj`)
