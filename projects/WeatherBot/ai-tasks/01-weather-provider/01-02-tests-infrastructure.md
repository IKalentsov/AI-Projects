...
**Что уже сформулировано у вендора — загрузить и следовать, а не изобретать:**
официальные скиллы `scaffold-dotnet-test-project` (подключение тестового проекта к решению),
`directory-build-organization` (иерархия `Directory.Build.props`, центральные версии),
`migrate-vstest-to-mtp` (свойства MTP, код выхода 8), `run-tests` (синтаксис `dotnet test`).
Обход ограничений песочницы — наш скилл `dotnet-verify-in-sandbox`.
...
Подробности — скилл `dotnet-verify-in-sandbox`.
...
- **интеграция с `dotnet test` выбирается по версии SDK и не смешивается:** на .NET 10 это
  секция `test` в `global.json` (`"runner": "Microsoft.Testing.Platform"`) — аргументы MTP
  передаются напрямую; `TestingPlatformDotnetTestSupport=true` — способ для .NET 9 и младше
  (режим VSTest-моста с разделителем `--`). Проект на .NET 10, поэтому выбран первый способ.
  **Если `global.json` ломает сборку — откатиться на второй и сказать об этом в отчёте;**
...
- [ ] Продакшн-код `src/**` не изменён — подтвердить `git status --short`.
